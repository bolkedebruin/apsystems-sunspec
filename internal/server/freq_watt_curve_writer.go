...
func (fwc *FreqWattCurveWriter) Apply(ctx context.Context, addrOffset uint16, regs []uint16) error {
	...
	// path appropriate for this inverter family.
	dispatch := func(paramName string, value float64) error {
		switch routeFor(family, paramName) {
		case RouteDirect:
			return fwc.writer.SetProtectionParam(ctx, uid, paramName, value)
		...
		case RouteUnsupported:
			return fmt.Errorf("%s on family %d (%s) is rejected at the inverter "+
				"firmware level; no host-side write path applies", paramName, inv.Model, uid)
		}
